...
class _CookProfileScreenState extends State<CookProfileScreen>
              ...
              BlocConsumer<CookFoodsManagerCubit, CookFoodsManagerState>(
                ...
                builder: (context, state) {
                  ...
                  return HorizontalListView(
                      itemCount:
                          topRatedFoods.isEmpty ? 4 : topRatedFoods.length,
                      itemBuilder: (index) {
                        return Padding(
                          padding: EdgeInsets.only(
                            left: index == 0
                          ...
                          child: FoodItem(
                            ...
                            onTap: () {
                              Navigator.of(context).pushNamed(
                                  Routes.foodDetailsRoute,
                                  arguments: {
                                    FoodDetailsScreen.foodIdKey: topRatedFoods
                                  });
                            },
                          ),
                        );
              ...
              BlocBuilder<CategoriesBloc, CategoriesState>(
                builder: (context, state) {
                  if (state is LoadedState) {
                    ...
                    BlocProvider.of<FoodsBloc>(context).add(GetAllFoodsEvent(
                        foodsGetParamsModel: FoodsGetParamsModel(
                    ...
                    children: [
                      ...
                      BlocBuilder<FoodsBloc, FoodsState>(
                        builder: (context, foodState) {
                          if (foodState is LoadedFoodsState) {
  ...
  Widget _buildFoods(context, CategoryEntity? category, foodState) {
    ...
    return foodState is LoadedFoodsState && foodState.foods.isEmpty
        ? const NoFoodsUi()
        : Column(
            children: List<Widget>.generate(
                foodState is LoadedFoodsState && isCurrentCategory
                ...
                (index) => Container(
                      ...
                      height: AppHeight.s200 * Constants.height,
                      child: FoodItem(
                        food: foodState is LoadedFoodsState && isCurrentCategory
